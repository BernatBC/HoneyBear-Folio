...
fn test_rename_account() {
    ...
    let account = crate::create_account_db(&db_path, "Old Name".to_string(), 0.0, "cash".to_string()).unwrap();
...
fn test_rename_account_empty_should_error() {
    let (_dir, db_path) = setup_db();
    let account = crate::create_account_db(&db_path, "Old".to_string(), 0.0, "cash".to_string()).unwrap();
