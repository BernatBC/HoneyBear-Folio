...
fn test_create_account_and_transaction_via_helpers() {
    ...
    let acc1 = crate::create_account_in_dir(&dir_path, "A".to_string(), 100.0, "cash".to_string()).unwrap();
    let acc2 = crate::create_account_in_dir(&dir_path, "B".to_string(), 0.0, "cash".to_string()).unwrap();

    assert_eq!(acc1.balance, 100.0);

    // Create a transfer transaction from acc1 to acc2
    let _tx = crate::create_transaction_in_dir(&dir_path, acc1.id, "2023-01-01".to_string(), acc2.name.clone(), Some("XFER".to_string()), None, -30.0).unwrap();
