...
fn test_create_account_and_transaction_via_helpers() {
    ...
    let acc1 = crate::create_account_in_dir(&dir_path, "A".to_string(), 100.0, "cash".to_string())
        .unwrap();
    let acc2 =
        crate::create_account_in_dir(&dir_path, "B".to_string(), 0.0, "cash".to_string()).unwrap();
