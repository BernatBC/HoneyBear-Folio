...
fn test_delete_account_with_transactions() {
    let (_dir, db_path) = setup_db();
    let account = crate::create_account_db(&db_path, "ToDelete".to_string(), 100.0, "cash".to_string()).unwrap();
    crate::create_transaction_db(&db_path, account.id, "2023-01-02".to_string(), "Payee".to_string(), None, None, -20.0).unwrap();
