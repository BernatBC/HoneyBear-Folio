use crate::{Account, calculate_account_balances};
...
fn test_custom_rate_application() {
    ...
    assert!((updated[0].exchange_rate - 1.1).abs() < 1e-6, "Exchange rate mismatch: expected 1.1, got {}", updated[0].exchange_rate);
}
...
fn test_cross_rate_conversion() {
    ...
    assert!((updated[0].balance - 80.0).abs() < 1e-6, "Balance mismatch: expected 80.0, got {}", updated[0].balance);

    // Exchange rate GBP -> USD should be 1.5
    assert!((updated[0].exchange_rate - 1.5).abs() < 1e-6, "Exchange rate mismatch: expected 1.5, got {}", updated[0].exchange_rate);
}
...
fn test_direct_rate_priority() {
    ...
    // 100 * 0.9 = 90.0
    assert!((updated[0].balance - 90.0).abs() < 1e-6, "Balance mismatch (direct rate): expected 90.0, got {}", updated[0].balance);
}
...
fn test_fallback_when_direct_rate_missing() {
    ...
    // 100 * (1.2 / 1.5) = 80.0
    assert!((updated[0].balance - 80.0).abs() < 1e-6, "Balance mismatch (pivot Yahoo): expected 80.0, got {}", updated[0].balance);
}
