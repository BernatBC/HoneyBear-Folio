...
async fn test_get_stock_quotes_all_network_fail_and_no_db_fallback() {
    ...
    let res_foo: Result<f64, _> = conn.query_row("SELECT price FROM stock_prices WHERE ticker = ?1", rusqlite::params!["FOO"], |r| r.get(0));
    let res_bar: Result<f64, _> = conn.query_row("SELECT price FROM stock_prices WHERE ticker = ?1", rusqlite::params!["BAR"], |r| r.get(0));
