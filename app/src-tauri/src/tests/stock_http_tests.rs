...
async fn test_search_ticker_success() {
    ...
    let resp = crate::search_ticker_with_client(client, server.base_url(), "AAPL".to_string()).await.unwrap();
...
async fn test_get_stock_quotes_with_partial_fail_and_db_fallback() {
    ...
    assert!(quotes.iter().any(|q| q.symbol == "BAA" && (q.price - 42.0).abs() < 1e-6));
